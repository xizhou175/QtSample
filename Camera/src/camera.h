...
public:
    ...
    Camera( const QVector3D& position = QVector3D(0.0f, 0.0f, 0.0f), const QVector3D& up = QVector3D(0.0f, 1.0f, 0.0f),
    ...
    Camera(QVector3D&& position, QVector3D&& up, float yaw = YAW, float pitch = PITCH)
        : Front(QVector3D(0.0f, 0.0f, -1.0f))
        , MovementSpeed(SPEED)
        , MouseSensitivity(SENSITIVITY)
        , Zoom(ZOOM)
